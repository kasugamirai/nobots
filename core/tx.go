...
func (tx *Tx) init() {
	tx.BotEvents = NewBotEventsClient(tx.config)
	tx.Events = NewEventsClient(tx.config)
	tx.NostrNotesFetchHistory = NewNostrNotesFetchHistoryClient(tx.config)
	tx.TwitterFetchHistory = NewTwitterFetchHistoryClient(tx.config)
	tx.TwitterNote = NewTwitterNoteClient(tx.config)
	tx.TwitterNotesFetchHistory = NewTwitterNotesFetchHistoryClient(tx.config)
	tx.TwitterProfile = NewTwitterProfileClient(tx.config)
}
...
// applies a query, for example: BotEvents.QueryXXX(), the query will be executed
